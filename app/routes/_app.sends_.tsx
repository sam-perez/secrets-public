/**
 * Sometimes you want the URL to be nested, but you don't want the automatic layout nesting.
    ...
    <>
      <section className="space-y-2 mb-8">
        ...
        {templates.map((template, index) => (
          <div key={index} className="flex items-center space-x-4 rounded-md hover:bg-slate-50 p-4">
            <div className="flex-1">
              <p className="font-medium">{template.name}</p>
              <p className="muted pb-0">
                {template.uses_number} uses · {template.security} · {template.last_updated}
              </p>
            </div>
      ...
      <Table>
        ...
        <TableBody>
          {sends.map((send, index) => (
            <TableRow key={index}>
              <TableCell className="py-6">{send.created}</TableCell>
